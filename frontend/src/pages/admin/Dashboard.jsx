      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        {summaryLoading && !cachedSummary
          ? Array.from({ length: 6 }).map((_, index) => (
              <div key={index} className="animate-pulse rounded-xl bg-white p-4 shadow-sm dark:bg-slate-950">
                <div className="h-3 w-20 rounded bg-slate-200 dark:bg-slate-800" />
                <div className="mt-3 h-7 w-14 rounded bg-slate-200 dark:bg-slate-800" />
              </div>
            ))
          : cards.map((card) => (
              <div key={card.label} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                <p className="text-sm font-medium text-slate-500 dark:text-slate-400">{card.label}</p>
                <p className="mt-2 text-2xl font-semibold text-slate-900 dark:text-slate-100">{formatCompactNumber(card.value)}</p>
              </div>
            ))}
      </div>
        ...
        >
          {summaryLoading && !cachedSummary ? (
            <div className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
              <div className="h-80 animate-pulse rounded-xl border border-slate-100 bg-slate-100 dark:border-slate-800 dark:bg-slate-900" />
              <div className="grid gap-3">
                ...
                <div className="h-52 animate-pulse rounded-xl border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-900" />
              </div>
            </div>
          ) : (
          <div className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
            <div className="h-80 rounded-xl border border-slate-100 bg-slate-50/50 p-2 dark:border-slate-800 dark:bg-slate-900/70 [&_.recharts-legend-item-text]:text-slate-600 dark:[&_.recharts-legend-item-text]:!text-slate-300">
            ...
            <div className="grid gap-3">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900">
                <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">Giải thích theo mô hình 2 lớp trạng thái</p>
                <div className="mt-3 grid gap-3 lg:grid-cols-2">
                  <div className="rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950">
                    <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">Tình trạng kỹ thuật</p>
                  ...
                  <div className="rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950">
                    <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">Vị trí sử dụng</p>
                    <div className="mt-2 space-y-2 text-sm text-slate-700 dark:text-slate-300">
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400">Vị trí gốc nghĩa là phòng ban, khu vực sở hữu tài sản. </p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="h-52 rounded-xl border border-slate-200 p-3 dark:border-slate-800 dark:bg-slate-950 [&_.recharts-cartesian-axis-tick_value]:fill-slate-500 dark:[&_.recharts-cartesian-axis-tick_value]:fill-slate-400">
        ...
        >
          <div className="space-y-3">
            {suggestionsLoading && suggestions.length === 0 && (
              <p className="text-sm text-slate-500 dark:text-slate-400">Đang tải gợi ý...</p>
            )}
            {suggestions.map((text, index) => (
              <div key={`${index}-${text}`} className="rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                <div className="flex items-start gap-3">
                  <span className="mt-0.5 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-orange-100 text-xs font-bold text-orange-700 dark:bg-orange-500/15 dark:text-orange-300">
                    {index + 1}
                  </span>
            ...
            {!suggestionsLoading && suggestions.length === 0 && (
              <p className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-6 text-center text-sm text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
      ...
      <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        >
          {helpdeskLoading && !cachedHelpdeskKpis ? (
            <div className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="h-24 animate-pulse rounded-xl border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-900" />
                <div className="h-24 animate-pulse rounded-xl border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-900" />
              </div>
              <div className="h-72 animate-pulse rounded-xl border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-900" />
            </div>
          ) : (
          <>
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Ticket đang hoạt động</p>
              <p className="mt-2 text-2xl font-semibold text-slate-800 dark:text-slate-100">{formatCompactNumber(helpdeskKpis?.activeTicketCount || 0)}</p>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Bao gồm ticket mới và đang xử lý</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Tỷ lệ quá hạn SLA</p>
              <p className="mt-2 text-2xl font-semibold text-slate-800 dark:text-slate-100">{formatPercentage(helpdeskKpis?.overdueSlaRate || 0)}</p>
          ...
          <div className="mt-4 h-72 rounded-xl border border-slate-200 p-3 dark:border-slate-800 dark:bg-slate-950 [&_.recharts-cartesian-axis-tick_value]:fill-slate-500 dark:[&_.recharts-cartesian-axis-tick_value]:fill-slate-400">
        ...
        >
          {helpdeskLoading && !cachedHelpdeskKpis ? (
            <div className="h-[22rem] animate-pulse rounded-xl border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-900" />
          ) : (
          <div className="h-[22rem] rounded-xl border border-slate-200 p-3 dark:border-slate-800 dark:bg-slate-950 [&_.recharts-cartesian-axis-tick_value]:fill-slate-500 dark:[&_.recharts-cartesian-axis-tick_value]:fill-slate-400 [&_.recharts-legend-item-text]:text-slate-600 dark:[&_.recharts-legend-item-text]:!text-slate-300">
